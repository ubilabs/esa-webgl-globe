// from https://github.com/ubilabs/esa-climate-from-space/blob/develop/src/scripts/libs/create-marker.ts
export async function getMarkerImage(title: string): Promise<HTMLCanvasElement> {
  const svgString = await getSvgString(unescape(encodeURIComponent(title)));
...
export async function getMarkerHTML(title: string): Promise<string> {
  return `
  <div class="myMarker" style="overflow: hidden"
...
async function getSvgString(storyTitle: string) {
  return `<svg class="myMarker" xmlns="http://www.w3.org/2000/svg" height="200" width="700">
